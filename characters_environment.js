...
noseX = ""; /* Aula 141. 1. */
noseY = ""; /* Aula 141. 1. */
GameStatus = ""; /* Aula 141. 3. */

function game(){

  console.log("noseX = " + noseX +" ,noseY =  "+ noseY); /* Aula 141. 2. */
  ...
  if(gameConfig.status==='start'){
    ...
    /* Aula 142. 5. */
    /* Aula 142. 6. rocure (Ctrl + f) por control.left e 
       escolha a 1ª opção dentro da function manualControl (characters). */
    textSize(40);
...
{
 if(noseX !="" && gameConfig.status==="start" && GameStatus=="start") /* Aula 142. 1, 2, 3, 4 */
 { 
...
function manualControl(character){
  ...
  if(character.live)
  {
    ...
    if(noseX > 300) /* Aula 142. 8.
                            142. 9. Ir para a functiom jumping(character) abaixo. */
    {
        character.velocity.x+=gameConfig.moveSpeed;
      character.changeAnimation('move');
      character.mirrorX(1);
